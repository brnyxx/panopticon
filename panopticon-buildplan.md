...
- **Novice first-run path.** Root help and terminal `doctor` output provide bilingual next-command guidance without changing JSON, result status, reason code, diagnostics, exit semantics, or command registration order. The pinned public path is `uv tool install panopticon-mcp==1.0.1` → `pano doctor --offline` → `pano watch SERVER_NAME --offline`. Mirrored EN/KO guides own prerequisites, installation, observation, result interpretation, commands, artifacts, cleanup, and troubleshooting; a dedicated agent guide owns JSON, exit-code, confirmation, and reporting contracts.
