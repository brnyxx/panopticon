...
from .watch_model import Coverage, Inventory, WatchOutcome, WatchRequest
from .watch_remote import RemoteObserver, RemoteTarget


class RuleExecutor(Protocol):
    def evaluate(self, evidence: object, *, target: object) -> tuple[object, ...]: ...


class McpDriver(Protocol):
    def drive(
        self, evidence: object, *, calls: int, args: tuple[str, ...], timeout: float, idle: float
    ) -> object: ...


class SpanCollector(Protocol):
    def collect(self, evidence: object) -> object: ...
...
class DeclaredExtractor(Protocol):
    def extract(self, evidence: object) -> object: ...


class Authority(Protocol):
    def apply(self, declared: object, *, target: object) -> object: ...
    ...
    def run(self, request: WatchRequest) -> tuple[WatchOutcome, ...]:
        ...
        for target in targets:
            if self._d.cancellation is not None and self._d.cancellation.cancelled():
                outcomes.append(
                    WatchOutcome(str(getattr(target, "name", target)), "INCOMPLETE", "CANCELLED")
                )
                break
            outcomes.append(self._run_target(target, request))
        return tuple(outcomes)

    def _run_target(self, target: object, request: WatchRequest) -> WatchOutcome:
        name = str(getattr(target, "name", target))
        used_local = False
        used_remote = False
        try:
            if (
                bool(getattr(target, "destructive", False))
            ...
            transport = str(
                getattr(
                    getattr(target, "transport", None),
                    "value",
                    getattr(target, "transport", "stdio"),
                )
            )
            if transport == "stdio":
                ...
                env = self._d.environment if request.options.real_env else {}
                local_run = self._d.local.run(
                    LocalTarget(name, command, tuple(getattr(target, "args", ())), env),
                ...
                used_remote = True
                endpoint = str(getattr(target, "url", ""))
            ...
            if self._d.spans is not None and evidence is not None:
                persistence += (self._d.spans.collect(evidence),)
            if self._d.events is not None and evidence is not None:
                persistence += (self._d.events.collect(evidence),)
