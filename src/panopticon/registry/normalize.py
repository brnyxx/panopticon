...
def _select(spec: str, versions: list[str]) -> str | None:
    ...
    if spec and not any(c in spec for c in "<>=^~*|"):
        return None
    candidates = [(v, tuple(map(int, _VERSION.fullmatch(v).groups()[:3]))) for v in versions if _VERSION.fullmatch(v)]
    if not candidates:
        return None
    ...
    def okay(t: tuple[int,int,int]) -> bool:
        for op, a, b, c in checks:
            ...
            if op == ">=" and not t >= base: return False
            if op == "<=" and not t <= base: return False
            if op == ">" and not t > base: return False
            if op == "<" and not t < base: return False
            if op == "^" and not (t >= base and t[0] == base[0]): return False
            if op == "~" and not (t >= base and t[:2] == base[:2]): return False
        return True
...
def normalize_npm(payload: Mapping[str, Any], *, name: str, spec: str, now: datetime | None = None) -> NormalizedHistory:
    return _normalize_registry("npm", payload, name=name, spec=spec, now=now)

def normalize_pypi(payload: Mapping[str, Any], *, name: str, spec: str, now: datetime | None = None) -> NormalizedHistory:
    return _normalize_registry("pypi", payload, name=name, spec=spec, now=now)

def normalize_github(payload: Mapping[str, Any], *, name: str, spec: str, now: datetime | None = None) -> NormalizedHistory:
    now = now or datetime.now(UTC)
    if not isinstance(payload, Mapping) or not isinstance(payload.get("releases"), list):
        return _result("github", name, spec, status=HistoryStatus.INCOMPLETE, reason_code=HistoryReason.MALFORMED_INPUT)
    ...
    for item in payload["releases"]:
        ...
        if reason: return _result("github", name, spec, status=HistoryStatus.INCOMPLETE, reason_code=reason)
        records.append(ReleaseRecord(version=ver, published_at=ts, age_days=_age(ts, now) if ts else None, yanked=bool(item.get("draft")), deprecated=False))
    records.sort(key=lambda r: r.version)
    resolved = _select(spec, [r.version for r in records])
    return _result("github", name, spec, status=HistoryStatus.AVAILABLE if resolved else HistoryStatus.UNKNOWN, reason_code=HistoryReason.OK if resolved else HistoryReason.UNRESOLVED_SPEC, resolved_version=resolved, source_url=_safe_url(payload.get("html_url") or payload.get("url")), releases=tuple(records))
...
def normalize_registry_history(
    ecosystem: str, payload: Mapping[str, Any], *, name: str, spec: str,
    ...
    return _result(ecosystem, name, spec, status=HistoryStatus.UNSUPPORTED, reason_code=HistoryReason.UNSUPPORTED_ECOSYSTEM)

def _normalize_registry(ecosystem: str, payload: Mapping[str, Any], *, name: str, spec: str, now: datetime | None) -> NormalizedHistory:
    now = now or datetime.now(UTC)
    if not isinstance(payload, Mapping):
        return _result(ecosystem, name, spec, status=HistoryStatus.INCOMPLETE, reason_code=HistoryReason.MALFORMED_INPUT)
    ...
    if ecosystem == "npm":
        ...
        latest = tags.get("latest") if isinstance(tags, Mapping) and isinstance(tags.get("latest"), str) else None
        raw = payload.get("maintainers")
        if isinstance(raw, list):
            maintainers = tuple(sorted(m["name"] for m in raw if isinstance(m, Mapping) and isinstance(m.get("name"), str)))
    else:
        info = payload.get("info", {})
        versions = payload.get("releases")
        times = {v: (items[0].get("upload_time_iso_8601") if items and isinstance(items[0], Mapping) else None) for v, items in (versions or {}).items()}
    ...
    for ver in sorted(versions):
        if not _version(ver): continue
        ts, reason = parse_timestamp(times.get(ver) if isinstance(times, Mapping) else None)
        if reason:
            return _result(ecosystem, name, spec, status=HistoryStatus.INCOMPLETE, reason_code=reason)
        ...
        records.append(ReleaseRecord(version=ver, published_at=ts, age_days=_age(ts, now) if ts else None, yanked=yanked, deprecated=dep))
    resolved = _select(spec, [r.version for r in records if not r.yanked and not r.deprecated])
    return _result(ecosystem, name, spec, status=HistoryStatus.AVAILABLE if resolved else HistoryStatus.UNKNOWN, reason_code=HistoryReason.OK if resolved else HistoryReason.UNRESOLVED_SPEC, resolved_version=resolved, latest=latest, maintainers=maintainers, archived=archived, source_url=_safe_url(source), releases=tuple(records))
