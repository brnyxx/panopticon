...
def lookup_history(
    ...
    *,
    ...
    if record is None:
        ...
        return NormalizedHistory(status=status, reason_code=reason, ecosystem=lookup.ecosystem, name=lookup.name, requested_spec=lookup.spec)
    ...
    if fetched.utcoffset() != timedelta(0) or fetched > now or now - fetched > max_age:
        ...
        return history.model_copy(update={"status": HistoryStatus.UNKNOWN, "reason_code": HistoryReason.STALE_CACHE})
    return record.history.model_copy(update={"registry_fetched_at": fetched, "registry_fresh": True})
...
def resolve_package_version(package: object, cache: Cache) -> str | None:
    ecosystem = getattr(getattr(package, "ecosystem", None), "value", getattr(package, "ecosystem", ""))
