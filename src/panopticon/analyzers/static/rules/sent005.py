...
from panopticon.analyzers.static.model import RuleRunState, SourceRange, StaticContext, StaticMatch

_KNOWN_SECRET = re.compile(
    r"(?:sk-[A-Za-z0-9_-]{20,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|xox[baprs]-[A-Za-z0-9-]{20,}|AKIA[A-Z0-9]{16}|AIza[A-Za-z0-9_-]{20,})"
)
_CONTEXTUAL_SECRET = re.compile(
    r"(?i)(?:api[_-]?key|secret|token|password|credential)\s*[:=]\s*(?P<quote>['\"]?)(?P<value>[A-Za-z0-9_./+=-]{20,})(?P=quote)"
)


def run(context: StaticContext, semgrep_candidates: list[StaticMatch], state: RuleRunState) -> None:
    candidate_paths = {item.path for item in semgrep_candidates}
    path_sources = {item.relative_path: item.source for item in context.files.python_files}
    ...
    for path in sorted(candidate_paths):
        ...
        for line_number, line in enumerate(source.splitlines(), 1):
            for match in _candidate_values(line):
                ...
                start, end = match.start(group) + 1, match.end(group) + 1
                ...
                state.matches.append(
                    StaticMatch(
                        ...
                        snippet=line.replace(value, "<redacted>"),
...
def _candidate_values(line: str) -> tuple[re.Match[str], ...]:
    return tuple(
        sorted(
            (*_KNOWN_SECRET.finditer(line), *_CONTEXTUAL_SECRET.finditer(line)),
...
def _is_reserved_test_path(path: str) -> bool:
    return Path(path).as_posix().startswith(("tests/fixtures/", "demo/"))
