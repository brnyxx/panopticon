...
def detect(context: StaticContext, state: RuleRunState) -> None:
    ...
    for file in context.files.python_files:
        ...
        for function in functions:
            ...
            for item in sorted(ast.walk(function), key=lambda node: getattr(node, "lineno", 0)):
                if isinstance(item, (ast.Assign, ast.AnnAssign)):
                    ...
                    if isinstance(value, ast.Call):
                        ...
                        if configured.intersection({resolved, local_name}) and _contains_names(
                            value, tainted
                        ):
                            tainted.update(names)
                ...
                if sink and _contains_names(item, tainted - sanitized):
                    state.matches.append(match_from_node("SENT-004", file, item, "prompt-taint"))
                    break
...
def _is_source(node: ast.AST) -> bool:
    if isinstance(node, ast.Attribute) and node.attr in {"text", "description", "content"}:
        return True
...
def _is_openai_sink(node: ast.AST) -> bool:
    ...
    if not name.endswith(("responses.create", "chat.completions.create")):
        return False
    return any(keyword.arg in {"input", "instructions", "messages"} for keyword in node.keywords)
