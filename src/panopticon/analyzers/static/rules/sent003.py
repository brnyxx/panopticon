                ...
                if unsafe is not None:
                    ...
                    if validation is not None and (
                        ...
                        state.matches.append(
                            match_from_node("SENT-003", file, unsafe, "untyped-parameter")
                        )
            else:
...
def _unsafe_parameter(node: ast.FunctionDef | ast.AsyncFunctionDef) -> ast.arg | None:
...
def _first_name_use(node: ast.AST, name: str) -> ast.Name | None:
    uses = [item for item in ast.walk(node) if isinstance(item, ast.Name) and item.id == name]
    return min(uses, key=lambda item: item.lineno) if uses else None
