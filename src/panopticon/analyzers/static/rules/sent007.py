...
class IntegrityEntry(BaseModel):
    ...
    def validate_mode(self) -> IntegrityEntry:
        ...
        signature_mode = all(
            value is not None for value in (self.public_key, self.signature, self.algorithm)
        )
        if hash_mode == signature_mode:
            raise ValueError("integrity entry must select exactly one verification mode")
...
def detect(context: StaticContext, state: RuleRunState) -> None:
    _load_anchors(context.configuration.scan_root)
    for file in context.files.python_files:
        for function in (
            ...
            for item in ast.walk(function):
                if isinstance(item, ast.Call) and _is_manifest_load(item):
...
def _load_anchors(root: Path) -> IntegrityManifest | None:
    ...
    if path.is_symlink() or not path.is_file():
        raise ValueError("sentinel.integrity.yaml must be a regular file")
    try:
        value = IntegrityManifest.model_validate(yaml.safe_load(path.read_text(encoding="utf-8")))
    except (OSError, TypeError, ValueError, yaml.YAMLError) as error:
        raise ValueError(f"invalid sentinel.integrity.yaml: {error}") from error
    for manifest, entry in value.manifests.items():
        if Path(manifest).is_absolute() or ".." in Path(manifest).parts:
            raise ValueError("integrity manifest paths must be repository-relative")
        for target in (entry.public_key, entry.signature):
            if target is not None and not (root / target).is_file():
                raise FileNotFoundError(f"integrity trust-anchor path does not exist: {target}")
    return value


def _is_manifest_load(node: ast.Call) -> bool:
    return (qualified_name(node.func) or "") in {
        "json.load",
        "json.loads",
        "yaml.safe_load",
        "yaml.load",
...
def _is_verification(node: ast.Call) -> bool:
    ...
    return name.endswith(("hashlib.sha256", "hmac.compare_digest", ".verify")) or name in {
        "hashlib.sha256",
        "hmac.compare_digest",
    }
