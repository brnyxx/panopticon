...
def detect(context: StaticContext, state: RuleRunState) -> None:
    manifest = getattr(context.configuration, "permissions", None)
    if manifest is None:
        ...
        for region in discover_tool_regions(file):
            ...
            if declared is None:
                state.matches.append(
                    match_from_node("SENT-001", file, region.node, "missing-tool-permissions")
                )
                continue
            ...
            for name, capability, used in capabilities:
                if not _is_broader(capability.scopes, used):
                    continue
                if capability.broad_scope_justification:
                    state.exempt(f"justified_{name}")
                    continue
                node = actual[3].get(name, region.node)
                state.matches.append(match_from_node("SENT-001", file, node, f"broad-{name}"))


def _actual_usage(node: ast.AST) -> tuple[set[str], set[str], set[str], dict[str, ast.AST]]:
    reads: set[str] = set()
    writes: set[str] = set()
    hosts: set[str] = set()
    ...
    for item in ast.walk(node):
        ...
        if name in {"open", "Path.open"} or name.endswith((".open", ".read_text", ".write_text")):
            argument = (
                item.args[0]
            ...
            is_write = name.endswith(".write_text") or bool(mode and any(c in mode for c in "wax+"))
            (writes if is_write else reads).add(path if path is not None else "<dynamic>")
            ...
            is_write = any(
                token in name for token in ("remove", "unlink", "move", "copy", "mkdir", "write")
            )
            (writes if is_write else reads).add(path if path is not None else "<dynamic>")
            evidence["filesystem.write" if is_write else "filesystem.read"] = item
        elif (
            name.startswith(("requests.", "httpx.", "aiohttp.", "urllib."))
            or name == "socket.create_connection"
        ):
            value = literal_string(item.args[0]) if item.args else None
            host = "<dynamic>" if value is None else (urlparse(value).netloc or value)
            hosts.add(host)
